    ...
    <div className="min-h-screen bg-slate-50 font-sans pb-20">
      ...
      <div className="bg-emerald-950 text-emerald-100 py-3 px-6 shadow-md border-b border-emerald-900/50">
         <div className="max-w-4xl mx-auto flex items-center justify-center gap-3 text-sm text-center">
            ...
            <p>
               <strong>Privacidad Blindada:</strong> Tus datos están protegidos bajo secreto profesional (Attorney-Client Privilege) y cifrado SSL de 256 bits. 
               <span className="block md:inline md:ml-1 text-emerald-300 font-medium">Jamás compartimos información con ICE ni agencias de control migratorio.</span>
            </p>
         </div>
      </div>
      ...
      <aside className="w-full md:w-72 bg-navy-950 text-white p-6 flex flex-col shrink-0 relative overflow-hidden">
        <div className="absolute top-0 right-0 w-32 h-32 bg-gold-500/10 rounded-full blur-[40px]"></div>
         <a href="/" className="text-emerald-400 hover:text-emerald-300 font-bold mb-8 block border border-emerald-500/30 w-fit px-4 py-2 rounded-lg bg-emerald-900/20 transition hover:bg-emerald-900/40">
            ← Volver al Portal
         </a>
         
         <div className="flex items-center gap-4 mb-10 relative z-10">
           <ShieldCheck className="text-gold-500" size={28} />
           <h2 className="text-xl font-serif font-bold text-white">Mi Caso Seguro</h2>
        </div>
        ...
        <nav className="space-y-2 flex-1 relative z-10">
          ...
          >
            <FileText size={20} /> Formulario de Admisión
          </button>
          <button 
          ...
          >
            <CheckCircle size={20} /> Tu Hoja de Ruta
          </button>
        </nav>

        <div className="relative z-10 mt-8 pt-6 border-t border-slate-800">
           <p className="text-xs text-slate-500 mb-2">Asistido legalmente por:</p>
        ...
        {step === 'intake' && (
          <div className="max-w-3xl mx-auto animate-in fade-in slide-in-from-bottom-4 duration-500">
             <header className="mb-10">
                <h1 className="text-3xl md:text-4xl font-serif font-bold text-navy-900 mb-3">Formulario de Admisión</h1>
                <p className="text-slate-600 text-lg">Por favor, ingrese sus datos biográficos reales. Esta información se utilizará para generar los borradores oficiales de sus trámites.</p>
             </header>
             ...
             <form onSubmit={handleContinue} className="bg-white p-8 rounded-2xl shadow-xl border border-slate-100">
                <div className="grid md:grid-cols-2 gap-6 mb-8">
                   ...
                   <div>
                      <label className="block text-sm font-bold text-slate-700 mb-2">Estado de Residencia Actual</label>
                   ...
                   <div>
                      ...
                      <p className="text-xs text-slate-500 mt-2 flex items-center gap-1">
                         ...
                         Solo te contactaremos de manera privada por llamada o WhatsApp cifrado de extremo a extremo.
                      </p>
                   </div>
                   
                   <div className="md:col-span-2">
                      <label className="block text-sm font-bold text-slate-700 mb-2">Dirección en EE. UU. <span className="text-slate-400 font-normal">(Opcional en esta evaluación)</span></label>
                   ...
                   <div className="md:col-span-2">
                      <label className="block text-sm font-bold text-slate-700 mb-2">A-Number <span className="text-slate-400 font-normal">(Opcional)</span></label>
                   ...
                   <div>
                      <label className="block text-sm font-bold text-slate-700 mb-2">Vía de Entrada a EE. UU.</label>
                      <select value={formData.entryType} onChange={e=>setFormData({...formData, entryType: e.target.value})} className="w-full border-slate-200 rounded-lg focus:ring-navy-500 focus:border-navy-500 bg-slate-50 px-4 py-3 font-semibold text-navy-800">
                         <option value="CBP">CBP One / Parole Humanitario</option>
                         <option value="VISA">Entrada con Visa</option>
                         <option value="EWI">Entrada sin inspección</option>
                      </select>
                   </div>
                   <div>
                      <label className="block text-sm font-bold text-slate-700 mb-2">Fecha Exacta de Entrada</label>
                ...
                <div className="bg-navy-50 p-5 rounded-xl border border-navy-100 mb-8">
                   <div className="flex items-start gap-4">
                      ...
                      <label htmlFor="consent" className="text-sm text-slate-700 cursor-pointer">
                         <strong className="text-navy-900 block mb-1">Consentimiento y Confidencialidad Legal</strong>
                         Entiendo que esta información está protegida por el privilegio abogado-cliente. Autorizo a <strong>{firmData.lawyerName}</strong> a utilizar estos datos exclusivamente para evaluar mi caso y generar mis formularios migratorios.
                      </label>
                   </div>
                </div>
                ...
                <div className="flex justify-end">
                   <button type="submit" disabled={!formData.consent} className={`px-8 py-4 rounded-xl font-bold flex items-center gap-3 transition shadow-lg ${formData.consent ? 'bg-navy-900 hover:bg-navy-800 text-white shadow-navy-900/20' : 'bg-slate-200 text-slate-400 cursor-not-allowed'}`}>
                      Continuar a Mi Hoja de Ruta <ChevronRight size={20} />
                   </button>
                </div>
             </form>
          </div>
        ...
        {step === 'roadmap' && (
          <div className="max-w-4xl mx-auto animate-in fade-in zoom-in-95 duration-500">
             <header className="mb-10 flex flex-col md:flex-row justify-between items-start md:items-end gap-6">
                <div>
                  <h1 className="text-3xl md:text-4xl font-serif font-bold text-navy-900 mb-3">Hoja de Ruta: {formData.firstName} {formData.lastName}</h1>
                  <p className="text-slate-600 text-lg">Estrategia definida basada en su ingreso: {formData.entryType === 'CBP' ? 'CBP One / Parole' : formData.entryType === 'EWI' ? 'Sin Inspección' : 'Visa'}</p>
                </div>
             </header>
             ...
             <section className="bg-white rounded-2xl shadow-sm border border-slate-200 p-8 mb-8 relative overflow-hidden">
                ...
                <h2 className="text-2xl font-bold text-navy-900 mb-6 flex items-center gap-3">
                   <FileText className="text-gold-500" /> Formularios Oficiales Requeridos
                </h2>
                <p className="text-slate-600 mb-6">El sistema ha seleccionado los siguientes formularios basados en su vía de entrada ({formData.entryDate}).</p>
             ...
             <div className="grid md:grid-cols-2 gap-8">
                ...
                <section className="bg-navy-950 text-white rounded-2xl shadow-xl p-8 relative overflow-hidden">
                   ...
                   <h2 className="text-xl font-bold text-gold-500 mb-6 flex items-center gap-3 relative z-10">
                      <MapPin size={24} /> Envíos USCIS ({formData.state})
                   </h2>
                   <div className="relative z-10">
                      <p className="text-slate-300 text-sm mb-4">Usted debe enviar su paquete físico completo a la siguiente dirección oficial:</p>
                ...
                <section className="space-y-6">
                   {formData.entryType === 'EWI' && (
                      <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-200">
                         <h3 className="text-lg font-bold text-navy-900 mb-3 flex items-center gap-2">
                            <AlertTriangle className="text-orange-500" size={20} /> Cortes EOIR
                         </h3>
                         <p className="text-slate-600 text-sm mb-4 leading-relaxed">
                            Al entrar sin inspección, revise constantemente el sistema automatizado al <strong>1-800-898-7180</strong> usando su A-Number: <span className="font-mono bg-slate-100 px-1">{formData.aNumber}</span>.
                         </p>
                      </div>
                   )}
                   ...
                   <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-200">
                      <h3 className="text-lg font-bold text-navy-900 mb-3 flex items-center gap-2">
                         <ShieldCheck className="text-emerald-500" size={20} /> Licencia en {formData.state}
                      </h3>
                      <p className="text-slate-600 text-sm mb-4 leading-relaxed">
                         Con el Notice of Action de USCIS, usted es elegible para solicitar su licencia temporal en el DMV de {formData.state}.
                      </p>
                      <button className="text-navy-600 font-bold text-sm hover:underline flex items-center gap-1">Ver Requisitos <ChevronRight size={16} /></button>
                   </div>
                </section>
             </div>
